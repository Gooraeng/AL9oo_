from __future__ import annotations
from discord import (
    app_commands, 
    Button, 
    ButtonStyle,
    DiscordException,
    Forbidden,
    Embed, 
    Interaction,
    TextChannel, 
    ui
)
from typing import (
    TYPE_CHECKING, 
    Union, 
    Literal,
    Optional, 
    List
)
from discord.ext import commands
from utils.embed_color import al9oo_point
from utils.commandpermission import permissioncheck

if TYPE_CHECKING:
    from al9oo import Al9oo

import asyncio



al9oo_channels = [1160568027377578034, 1161584379571744830]
arn_channel = 1188494656170901546



class AlertHelpView(ui.View):
    def __init__(self,
                 embeds : List[Embed] = None
                 ):
        super().__init__(timeout= None)
        self._embeds = embeds
        try:
            self.children[0].disabled = True
        except:
            if self.children[0].disabled is True:
                self.children[1].disabled = False
            else:
                self.children[1].disabled = True

    async def button_update(self, num, interaction : Interaction):
        self.children[num].disabled = True
        for i in range(0, len(self.children)):
            if num != i:
                self.children[i].disabled = False              
        await interaction.response.edit_message(embed= self._embeds[num], view= self)


    @ui.button(label= 'Clear', style= ButtonStyle.blurple, custom_id= 'help-al9oo-noti', row= 0)
    async def Clear(self, interaction : Interaction, button : Button):
        await self.button_update(num= 0, interaction= interaction)


    @ui.button(label= 'Set', style= ButtonStyle.blurple, custom_id= 'help-a9-releasenote', row= 0)
    async def Set(self, interaction : Interaction, button : Button):
        await self.button_update(num= 1, interaction= interaction)



@app_commands.default_permissions(administrator = True)
@app_commands.guild_only()
class AnnounceAL9oo(commands.GroupCog, name= 'alert'):
    def __init__(self, app : Al9oo) -> None:
        self.app = app


    async def follow_al9oo(self, interaction : Interaction, target : TextChannel):
        channels = al9oo_channels
        webhook_ids = []
        done = []
        webhooks = await interaction.guild.webhooks()
        
        if not webhooks:
            for a in channels:
                departure = await interaction.client.fetch_channel(a)
                await departure.follow(destination= target)
                done.append(departure.mention)

        else:
            for webhook in webhooks:
                if webhook.source_channel is None:
                    continue
                webhook_ids.append(webhook.source_channel.id)
            
            for a in channels:
                if a not in webhook_ids:
                    departure = await interaction.client.fetch_channel(a)
                    await departure.follow(destination= target)
                    done.append(departure.mention)
        return done


    async def follow_arn(self, interaction : Interaction, target : TextChannel):
        channel = arn_channel
        webhook_ids = []
        done = []
        webhooks = await interaction.guild.webhooks()

        if not webhooks:
            departure = await interaction.client.fetch_channel(channel)
            await departure.follow(destination= target)
            done.append(departure.mention)

        else:
            for webhook in webhooks:
                if webhook.source_channel is None:
                    continue
                webhook_ids.append(webhook.source_channel.id)
            
            if channel not in webhook_ids:
                departure = await interaction.client.fetch_channel(channel)
                await departure.follow(destination= target)
                done.append(departure.mention)
                
        return done


    @app_commands.command(name= 'help', description= "Get small tip of alert commands!")
    async def help(self, interaction : Interaction):
        try:
            embed1 = Embed(title = "/alert clear",
                description= "Only remove webhooks that AL9oo created!",
                color= al9oo_point)
            embed1.add_field(name= "Don't worry!", value= "I don't delete any webhook that I didn't make")
            
            embed2 = Embed(
                title= "/alert set",
                description= "Create webhook(s) to get notifications in AL9oo announcements or Asphalt 9 Release Notes.",
                color= al9oo_point)
            embed2.add_field(name= "1. AL9oo", value= "Get nofitications about my update, inspection, db update.")
            embed2.add_field(name= "2. Asphalt 9 Release note", value= "Get notifications from Asphalt 9 Release Notes faster than anyone else!")
            embeds = [embed1, embed2]
            
            view = AlertHelpView(embeds= embeds)
            return await interaction.response.send_message(view= view, embed= embed1, ephemeral= True)
        
        except Exception as e:
            await interaction.response.defer(thinking= True, ephemeral= True)
            await interaction.followup.send(content= "Sorry, something error occured.", ephemeral= True)
            raise e


    @app_commands.command(name= 'set', description= "Create webhook(s) to get notifications in AL9oo announcements or Asphalt 9 Release Notes.")
    @app_commands.describe(target = "Choose Channel", select = 'What do you want follow?')
    @app_commands.rename(target = "channel")
    @app_commands.checks.cooldown(3, 30, key= lambda i : (i.guild_id, i.user.id))
    @app_commands.checks.bot_has_permissions(manage_webhooks = True)
    async def set_channel(self, interaction : Interaction, target : TextChannel, select : Optional[Literal["AL9oo", "Asphalt 9 Release note"]] = None):        
        await interaction.response.defer(thinking= True, ephemeral= True)
        try:
            match select:
                case "AL9oo":
                    done_1 = await self.follow_al9oo(interaction= interaction, target= target)
                    done_2 = []
                case "Asphalt 9 Release note":
                    done_1 = []
                    done_2 = await self.follow_arn(interaction= interaction, target= target)
                case None:
                    done_1 = await self.follow_al9oo(interaction= interaction, target= target)
                    done_2 = await self.follow_arn(interaction= interaction, target= target)
            
            done = done_1 + done_2
            
            if not done:
                return await interaction.followup.send(embed= Embed(
                    title= "Warn", description= f"You are already following all announcements.\nif you want to change setup, Run `/alert clear`, then run `/alert set`.", color= al9oo_point
                ), ephemeral= True)
            
            else:           
                if done:
                    followed = "\n* " + "\n* ".join(s for s in done)
                else:
                    followed = ""
        
                return await interaction.followup.send(embed= Embed(
                    title= "Warn", description= f"You will receive notifications here from now.\n## {target.mention}\n### Following Channel" + followed, color= al9oo_point
                ), ephemeral= True)

        except Exception:
            await asyncio.sleep(5)
            return await interaction.followup.send(f"Failed run this command due to error.\nFirst, please check whether I can see {target.mention}", ephemeral= True)

    
    @app_commands.command(name= 'delete', description= "Only remove webhooks that AL9oo created!")
    @app_commands.describe(select = "Select one, or not, I'll clear all webhooks that I made!")
    @app_commands.checks.cooldown(3, 30, key= lambda i : (i.guild_id, i.user.id))
    @app_commands.checks.bot_has_permissions(manage_webhooks = True)
    async def clear_channel(self, interaction : Interaction, select : Optional[Literal["AL9oo", "Asphalt 9 Release note"]] = None):        
        await interaction.response.defer(thinking= True, ephemeral= True)         
        webhooks = await interaction.guild.webhooks()
        
        if not webhooks:
            return await interaction.followup.send("It's clear already.", ephemeral= True)
        
        if select == "AL9oo":
            channels = al9oo_channels
        elif select == "Asphalt 9 Release note":
            channels = [arn_channel]
        else:
            channels = al9oo_channels + [arn_channel]
        
        deleted = []; failed = []
        for webhook in webhooks:
            source = webhook.source_channel
            if source is not None:
                for i in channels:
                    if source.id == i:
                        target = await interaction.client.fetch_channel(i)
                        try:
                            await webhook.delete()
                            deleted.append(target.mention)
                        except Exception:
                            failed.append(target.mention)
                            
        if not deleted:
            return await interaction.followup.send("It's clear already.", ephemeral= True)
        
        elif not failed:
            deleted = "\n* ".join(s for s in deleted)
            return await interaction.followup.send(embed= Embed(
                title= "Warn", description= f"Deleted webhooks.", color= al9oo_point
            ), ephemeral= True)
        
        else:
            failed = "\n* ".join(s for s in failed)
                
            return await interaction.followup.send(embed= Embed(
                title= "Warn", description= f"Deleted webhooks Except for...\n* {failed}\nYou may delete manually for run this command again without any choice.", color= al9oo_point
            ), ephemeral= True)


    @clear_channel.error
    @set_channel.error
    async def alert_err(self, interaction : Interaction, error : Union[app_commands.AppCommandError, DiscordException]):
        if isinstance(error, app_commands.BotMissingPermissions):
            await permissioncheck(interaction= interaction, error= error)
        if isinstance(error, app_commands.CommandOnCooldown):
            await interaction.response.send_message(f"Too much requests! Please wait for {int(error.retry_after)} second(s)!")
        if isinstance(error, Forbidden):
            raise error


async def setup(app : Al9oo):
    await app.add_cog(AnnounceAL9oo(app))